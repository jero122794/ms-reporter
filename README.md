...
![Intro](docs/images/ms-reporter_intro.png "Intro")
# Table of Contents
  * [Project Structure](#structure)
  * [FrontEnd](#frontend)
    *  [Environment variables](#frontend_env_vars)
  * [API](#api)
    * [GraphQL throught reporter-ui-gateway API](#api_reporter-ui-gateway_graphql)
  * [BackEnd](#backend)
    *  [reporter](#backend_reporter)
        * [Environment variables](#backend_reporter_env_vars)
        * [Event Sourcing](#backend_reporter_eventsourcing)
        * [CronJobs](#backend_reporter_cronjobs)
  * [Development environment](#dev_env)
# Project structure <a name="structure"></a>

```
├── frontend                            => Micro-FrontEnd  
│   └── reporter-ui                      => Micro-FrontEnd for [reporter-ui FrontEnd](https://github.com/git_rep_directoryo_directory_path/reporter-ui)
├── api                                 => Micro-APIs  
│   └── reporter-ui-gateway                           => Micro-API for [reporter-ui-gateway API](https://github.com/nebulae-university/reporter-ui-gateway)  
├── backend                             => Micro-BackEnds  
│   ├── reporter                     => Micro-BackEnd responsible for ...
...
## GraphQL throught reporter-ui-gateway API <a name="api_reporter-ui-gateway_graphql"></a>
These are the exposed GraphQL functions throught the [reporter-ui-gateway API](https://github.com/nebulae-university/reporter-ui-gateway).  

Note: You may find the GraphQL schema [here](api/reporter-ui-gateway/graphql/reporter/schema.gql)
...
#### reporterHelloWorldSubscription
...
## reporter <a name="backend_reporter"></a>
...


### Environment variables <a name="backend_reporter_env_vars"></a>
...
```
#### Notes: 
  * ENV VARS for development are [here](backend/reporter/.env)
  * ENV VARS for production are [here](deployment/gke/deployment-reporter.yaml)

### Event Sourcing <a name="backend_reporter_eventsourcing"></a>
...
### CronJobs <a name="backend_reporter_cronjobs"></a>
...
### 1. clone this repo  
   ```git clone https://github.com/nebulae-university/ms-reporter.git```  
...
### 5. Compose FrontEnd
```
nebulae compose-ui development --shell-type=FUSE2_ANGULAR --shell-repo=https://github.com/nebulae-university/reporter-ui --frontend-id=reporter-ui --output-dir=/FULL_PATH_TO_REPO/ms-reporter/playground/reporter-ui  --setup-file=/FULL_PATH_TO_REPO/ms-reporter/etc/mfe-setup.json
```

### 6. Compose the API reporter-ui-gateway
```
nebulae compose-api development --api-type=NEBULAE_GATEWAY --api-repo=https://github.com/nebulae-university/reporter-ui-gateway --api-id=reporter-ui-gateway --output-dir=FULL_PATH_TO_REPO/ms-reporter/playground/reporter-ui-gateway  --setup-file=FULL_PATH_TO_REPO/ms-reporter/etc/mapi-setup.json
```
...
* set this key value to the **JWT_PUBLIC_KEY** atribute in the following files: *WORKING_FOLDER*/ms-reporter/backend/reporter/.env   *WORKING_FOLDER*/ms-reporter/playground/reporter-ui-gateway/.env  
...
* Setup the Apollo engine key to trace API requests
    * create a key at https://engine.apollographql.com/ and set it to the **APOLLO_ENGINE_API_KEY** atribute in the playground/reporter-ui-gateway/.env file

### 8. Remove FrontEnd base href used on production
change ```<base href="/reporter-ui/">``` to ```<base href="/">``` in the index.html located at playground/reporter-ui/src/index.html
...
npm start
```
3. Start the API reporter-ui-gateway
```
cd playground/reporter-ui-gateway
npm run start-dev-env
```
4. Start the FrontEnd
```
cd playground/reporter-ui
npm run start-dev-env
```
